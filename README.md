...
```sh
curl -L -O https://github.com/bzkf/onco-analytics-on-fhir/releases/download/v2.2.3/air-gapped-installer.tgz
```
...
```sh
helm upgrade --install --wait --timeout=10m --version=2.2.3 prerequisites oci://ghcr.io/bzkf/onco-analytics-on-fhir/charts/prerequisites
...
```sh
helm upgrade --install --wait --timeout=10m --version=2.2.3 onco-analytics-on-fhir oci://ghcr.io/bzkf/onco-analytics-on-fhir/charts/onco-analytics-on-fhir
